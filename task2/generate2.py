...
# 文件路径
A_PATH = '/data/user/jzt/crd/audioLLM/task2/fillwave/tau/fill_front_5.wav'
B_PATH = '/data/user/jzt/crd/audioLLM/task2/fillwave/tau/fill_back_5.wav'
C_PATH = '/data/user/jzt/crd/audioLLM/task2/fillwave/tau/fill_mid.wav'
...
OUTPUT_AUDIO_DIR = 'tau02/'
MANIFEST_CSV = 'tau02/output_manifest.csv'
SOURCE_AUDIO_DIR = '/data/user/jzt/crd/audioLLM/foa_dev/foa_dev/dev-train-tau/'

# 目标类别集合
TARGET_CLASSES = {4, 12, 7, 6, 2, 11, 3}
...
def process_audio_events(df: pd.DataFrame):
    ...
    for _, row in df.iterrows():
        ...
        if class_label in TARGET_CLASSES:
            ...
            # 如果事件音频小于7秒，重复音频直到超过7秒
            while event_duration < 7:
                event_audio += original_audio[int(onset * 1000):int(offset * 1000)]  # 重复原始事件音频片段
                event_duration = len(event_audio) / 1000  # 更新重复后音频的时长
